...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    try {
        await sendEmail({
            email: user.email,
            subject: 'Your password token (valid for 10mins)',
            message
        });
        ...
        await user.save({ validateBeforeSave: false });
        // return next(new AppError('There was a error sending to email. Try again later.', 404))
        res.status(500).json({
            status: 'fail',
            error: error
        })
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    // 1) Get user based on the token via Params
